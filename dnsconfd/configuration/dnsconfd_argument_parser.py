...
class DnsconfdArgumentParser(ArgumentParser):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._config_values = [
            ...
            BoolOption("handle_service",
                       "Indicates whether dnsconfd should handle underlying "
                       "cache service life cycle, if disabled then dnsconfd "
                       "will not be able to start/restart/stop service and "
                       "will rely that the service is already running",
                       True,
                       in_file=True,
                       in_args=True,
                       in_env=True)
    ...
    def add_commands(self):
        ...
        make_resolvconf = (
            config_subparse.add_parser("make_resolvconf",
                                       help="Print resolvconf file according "
                                            "to config"))

        make_resolvconf.set_defaults(
            func=lambda: Cmds.make_resolvconf(vars(self._parsed)))

        make_unboundconf = (
            config_subparse.add_parser("make_unboundconf",
                                       help="Print unbound configuration "
                                            "according to config"))
        make_unboundconf.set_defaults(
            func=lambda: Cmds.make_unboundconf(vars(self._parsed)))
