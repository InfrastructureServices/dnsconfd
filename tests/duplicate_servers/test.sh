...
rlJournalStart
    rlPhaseStartSetup
        ...
        rlRun "dnsconfd_cid=\$(podman run -d --dns='none' --cap-add=NET_ADMIN --cap-add=NET_RAW --network dnsconfd_network1:interface_name=eth0,ip=192.168.5.2 --network dnsconfd_network2:interface_name=eth1,ip=192.168.6.2 dnsconfd_testing:latest)" 0 "Starting dnsconfd container"
