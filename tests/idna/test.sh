...
rlJournalStart
    rlPhaseStartSetup
        ...
        rlRun "dnsconfd_cid=\$(podman run --privileged -d --dns='none' --privileged --network dnsconfd_network:interface_name=eth0,ip=192.168.6.2 --network dnsconfd_network2:interface_name=eth1,ip=192.168.7.2 dnsconfd_testing:latest)" 0 "Starting dnsconfd container"
