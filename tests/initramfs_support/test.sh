    ...
    rlPhaseStartTest
        if [ "${TMT_GUEST[name]}" = "client" ]; then
            if [ "$TMT_REBOOT_COUNT" -eq 0 ]; then
              ...
              rlRun "sed -i 's/\(GRUB_CMDLINE_LINUX.*=\".*\)\"/\1 rd.neednet rd.net.dns=dns+tls:\/\/${TMT_GUESTS[server.hostname]}#named ip=dhcp rd.net.dns-resolve-mode=exclusive rd.net.dns-backend=dnsconfd\"/g' /etc/default/grub"
              rlRun "grubby --update-kernel=/boot/vmlinuz-$(uname -r) --args=\"rd.neednet rd.net.dns=dns+tls://${TMT_GUESTS[server.hostname]}#named ip=dhcp rd.net.dns-resolve-mode=exclusive rd.net.dns-backend=dnsconfd\""
              rlRun "dracut -vvv --force" 0 "Rebuild initramfs"
