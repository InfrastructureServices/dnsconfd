...
def test_server_description():
    ...
    l4 = network_objects.ServerDescription(int(socket.AF_INET), BIN_LOCALHOST4)
    assert(l4.to_unbound_string() == LOCALHOST4)

    l6 = network_objects.ServerDescription(int(socket.AF_INET6), BIN_LOCALHOST6)
    assert(l6.to_unbound_string() == LOCALHOST6)

    ls4 = network_objects.ServerDescription(int(socket.AF_INET), BIN_LOCALHOST4, port=853, sni='server.test')
