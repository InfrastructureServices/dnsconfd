...
static unbound_conf_tuple unbound_conf_cases[] = {
    ...
    {{{"*", {"dns+tls://[2001:db8::1]:55#example.org", NULL}}, {0}},
     "server:\n\tmodule-config: \"ipsecmod iterator\"\n\tinterface: "
     "127.0.0.1\n\tdo-not-query-address: 127.0.0.1/8\nforward-zone:\n\tname: "
     "\".\"\n\tforward-addr: 2001:db8::1@55#example.org\n\tforward-tls-upstream: yes\n"},
...
START_TEST(test_get_unbound_conf_string) {
  ...
  // size of compile-time calculation here, so we can use NULL as value to indicate no output
  for (int i = 0; i < sizeof(unbound_conf_cases) / sizeof(unbound_conf_tuple); i++) {
    ...
    resolvconf_content = get_unbound_conf_string(glob_dict, devnull);
    if (cur_output != NULL) {
